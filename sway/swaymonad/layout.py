...
def increase_mfact_dispatcher(i3: i3ipc.Connection, event: i3ipc.Event):
  del event
  workspace = common.get_focused_workspace(i3)
  master = common.find_biggest_window(workspace)
  if not master:
    return
  master.command("resize shrink width 20px")


def decrease_mfact_dispatcher(i3: i3ipc.Connection, event: i3ipc.Event):
  del event
  workspace = common.get_focused_workspace(i3)
  master = common.find_biggest_window(workspace)
  if not master:
    return
  master.command("resize grow width 20px")
